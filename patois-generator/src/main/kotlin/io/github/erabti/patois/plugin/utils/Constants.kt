package io.github.erabti.patois.plugin.utils

object Constants {
    internal const val DEFAULT_CLASS_NAME = "AppStrings"
    internal const val DEFAULT_ENUM_NAME = "AppLocale"

    internal const val DEFAULT_RESOLVER_NAME = "AppLocaleResolver"

    internal const val DEFAULT_INPUT_DIR = "src/main/resources/translations"
    internal const val DEFAULT_OUTPUT_DIR = "generated/source/patois/main/kotlin"

    internal const val PATOIS_CORE_PACKAGE = "io.github.erabti.patois.models"
}